			...
			{if $output.counter.overzicht > 0}
				{foreach from=$output.overzicht item=andere}
					<tr class="
						...
						{elseif $andere.color eq 0}even{/if}
						
						{if $andere.afgemeld eq 1}afgemeld{/if}
						
						pointer"
					    
						{if $andere.actie != ""}
							onmouseover='return tooltip("{$andere.actie}","","bordercolor:black, border:1, backcolor:#d3d3d3");'
							onmouseout='return hideTip();'
						{/if}
					    
						onclick="window.open('toppiedesk.php?id={$output.page.id}&searchoid={$andere.id}','_blank');">
						<td>{$andere.naam}</td>
						<td>{$andere.datum}</td>
						<td>{$andere.categorie}</td>
						<td class="probleem">{$andere.probleem}</td>
						<td>{$andere.counter}</td>
						<td>{$andere.status}</td>
