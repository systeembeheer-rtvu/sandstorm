<?php
    ...
    class IncidentNieuw
    {
	    ...
	    $page['settings']['locations']['default_site']['header'] = "headers/blankheader";
	    $page['settings']['locations']['default_site']['sidebar'] = "sidebars/printsidebar";
            
            $output["navbar"]["new"] = $page['settings']['locations']['file'] . "?id=callnieuw"; //waarde voor een nieuw object
            $output["navbar"]["save"] = $page['settings']['locations']['file'] . "?id=callnieuw"; //waarde voor het opslaan van een object
            ...
            $output['navbar']['id'] = "incidentzoeken";
        }
	    ...
	    $output['userid'] = $_COOKIE['behandelaar'];
        }
        ...
        function Opslaan()
        {
            ...
            $output['redirect'] = $page['settings']['locations']['file'] ."?id=callnieuw&status=saved&saveid=" . $output['oid']; 
        }
        
        function IncidentToProject()
        {
            global $input, $page, $output;
            ...
            $output['redirect'] = $page['settings']['locations']['file'] ."?id=projectnieuw&searchoid=" . $output['project']; 
        }
        ...
        function toProject()
        {
            ...
            $project['titel'] = "Call " . $output['oid'];
            $project['doel'] = $input['probleem'];
            $project['aangemaaktop'] = date("Y-m-d", mktime(0,0,0,date('m'),date('d'),date('Y'))) . " 00:00:00";
            $project['aangemaaktdoor'] = $_COOKIE['behandelaar'];
            
            $queryvars = array($project['titel'], $project['doel'], $project['aangemaaktop'], $project['aangemaaktdoor']);
            $query = "
            ...
            $output['project'] = $page['classes']['sql']->last_insert_id(__line__,__file__);
            
            $project['omschrijving'] = "Project gemaakt van call " . $output['oid'];
            
            $queryvars = array($output['project'], $project['aangemaaktdoor'], $project['omschrijving'], $project['aangemaaktop']);
            $query = "
        ...
        function incidentOpslaan()
        {
            ...
            if($input['oid'] > 0)
            {
                ...
                $query = "
                    ...
                    SET naam = ?, telefoonnummer = ?, afdeling = ?, probleem = ?, status = ?, afgemeld = ?, categorie = ?, asset = ?
                ";
                ...
                if($input['oldafgemeld'] == 0 && $input['afgemeld'] == 1)
                    $this->IncidentAfmelden();
                else if($input['oldafgemeld'] == 1 && $input['afgemeld'] == 0)
                    $this->incidentGeopend();
            }
            else if($input['oid'] == 0)
            {
                $queryvars= array(
        ...
        function actieToevoegen()
        {
            ...
            if($input['actie'] != "" && $output['oid'] != 0)
            {
                $queryvars= array(
                    ...
                    $_COOKIE['behandelaar'],
                    $input['inputTime'],
                    $this->TimeToInt($input['bestedetijd'])
                );
                $query = "
                    INSERT INTO " . $page['settings']['locations']['db_prefix'] . "incidentacties (incident_id, actie, behandelaar, datum, bestedetijd)VALUES (?, ?, ?, ?, ?)
                ";
        ...
        function actieBijwerken()
        {
            ...
            if($input['ai_id'] != "")
            {
                foreach($input['ai_id'] as $ai_id)
                {
                    ...
                    $tijd = $this->TimeToInt($_POST['ai_tijd'][$ai_id]);
                    
                    
                    $queryvars = array($value, $tijd, $ai_id);
                    $query = "UPDATE " . $page['settings']['locations']['db_prefix'] . "incidentacties SET actie = ?, bestedetijd = ? where id = ?";
        ...
        function mailToewijzing($behandelaar)
        {
            ...
            $mailOutput['oid'] = $output['oid'];
            $mailOutput['aanmelder'] = $input['naam'];
            $mailOutput['telefoonnummer'] = $input['tel'];
            $mailOutput['categorie'] = $vars['categorie'];
            $mailOutput['melding'] = utf8_decode($input['probleem']);
            ...
            $mailOutput['behandelaar'] = $vars['naam'];
            ...
            $mailOutput['invoerder'] = $vars['naam'];
            ...
            $smarty->assign("output", $mailOutput);
            ...
            $page['classes']['phpmailer']->Subject = "(Call toegewezen)(" . $mailOutput['categorie'] .")...";
        ...
        function IncidentWeergeven()
        {
            ...
            while($vars = $page['classes']['sql']->fetch_array($sth))
            {
                ...
                $output['incident']['asset']        = $vars['asset'];
        ...
        {
            global $input, $page, $output;
            $queryvars = array($output['oid'], "Call afgemeld", $_COOKIE['behandelaar']);
            $query = "
                INSERT INTO " . $page['settings']['locations']['db_prefix'] . "incidentacties (
                    incident_id,
                    actie,
        ...
        function incidentGeopend()
        {
            global $input, $page, $output;
            
            $queryvars = array($output['oid'], "Incident geopend", $_COOKIE['behandelaar']);
            $query = "
                INSERT INTO " . $page['settings']['locations']['db_prefix'] . "incidentacties (
                    incident_id,
                    actie,
        ...
        function OverigeIncidenten()
        {
            ...
            while($vars = $page['classes']['sql']->fetch_array($sth))
            {
                ...
                $vars['aangemeld'] = $dagArr[$dag] . "<br />" . date("d-m-Y" ,$vars['aangemeld']) . "<br />" . date("H:i", $vars['aangemeld']);
