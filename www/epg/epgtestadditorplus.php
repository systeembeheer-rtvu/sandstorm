...
// MySQL query
$epgtabel = "
    SELECT ProgDate, ProgStartTime, ProgTitle, ProgItemID, ProgHHID, ProgMediaFile, Published, ShowInMissed, ProgId,ProgDeleted,daletid
    FROM epg_test
...
TABLEEPG;
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $progdeleted = filter_var($row['ProgDeleted'],FILTER_VALIDATE_BOOLEAN);
        echo "<tr>
            <td>{$row['ProgDate']}</td>
            <td>{$row['ProgStartTime']}</td>
            <td>{$row['ProgTitle']} {$row['daletid']}</a></td>
            <td>" . ($origineel ? 'Nee' : 'Ja') . "</td>
            <td>" . ($row['Published'] ? 'Ja' : 'Nee') . "</td>
            <td>" . ($gemist ? 'Ja' : 'Nee') . "</td>
            <td>" . ($progdeleted ? 'Ja' : 'Nee') . "</td>
        </tr>\n";
    }
